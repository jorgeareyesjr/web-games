...
export default function LightThemesMenu() {
	...
	const buildMenu = () => (
		<menu className={`c-menu c-menu--light-themes`}>
			<h3 className={`c-title`}>Light Themes</h3>
			<ul className={`c-list`}>
				{
					activeTheme && themes
						.filter((theme: ITheme) => theme.base === 'light')
						.map((theme: ITheme) => (
							<li
								key={theme.id}
								className={setListingClassName(activeTheme, theme)}
								// style={getThemeListingBackground(theme)}
